    ...
    for i in range(l):
        if pred_1[i] == pred_2[i]:
            if pred_1[i] == 0:  # PO, and, PI
                pred.append(torch.tensor([1], device=out1.device, dtype=pred_1.dtype))
            else:  # maj, xor
                pred.append(pred_1[i] + 2)  # 3 or 4
        else:
            if (pred_1[i] == 1 and pred_2[i] == 2) or (
                pred_1[i] == 2 and pred_2[i] == 1
            ):
                pred.append(torch.tensor([2], device=out1.device, dtype=pred_1.dtype))  # maj and xor
            else:
